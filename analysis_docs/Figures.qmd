...
```{r}
#| include: false
all_data_wide <- read.csv2("data/master_database_v5.csv")
```
...
```{r}
#| echo: false
nirs_panel <- arm_oxy_plot / arm_deoxy_plot / leg_oxy_plot / leg_deoxy_plot +
    plot_annotation(tag_levels = 'A', tag_suffix = '.')
```

```{r}
#| echo: false
ggsave(plot = nirs_panel, filename = "nirs_panel.svg", 
...
peroxide_plot <- 
  ...
  stat_summary(fun = "mean", geom = "point", size = 3.2, stroke = 1, 
               ...
               shape = 21, fill = "white") +
  theme_rbc3() +
  labs(x = "Hydrogen peroxide (nM)", y = "Glycolytic flux (μM/min)") +
  scale_color_manual(values = rbc.palette1)
